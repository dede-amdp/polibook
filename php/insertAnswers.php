<?php
    ...
    if(isset($_POST['id'])) {
        ...
        $table = 'questionario (risposte, id_esame_corso, id_esame_attdid, ord_esame_attdid, id_esame_docente)';
        $risposte = '';
        foreach($_POST as $key => $value){
            if($key != 'id'){
                $risposte = $risposte."<$key>$value</$key>";
            }
        }
        if(verify($conn, $id)){
            if(insert_DB($conn, $table, $risposte, ...$id)){
