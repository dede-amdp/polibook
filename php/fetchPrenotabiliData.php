    ...
    while($result && $row=mysqli_fetch_assoc($result)){
        ...
        foreach($count_iscritti as $array){
            ...
            $array['ord_attdid_esame'] == $row['ordinamento']){
                $n_iscritti = $array['numero iscritti']; 
                break;
            }
        }
        $to_add = array_slice($row, 0, 10, true) + array('numero iscritti' => $n_iscritti) + array_slice($row, 10, count($row) - 1, true);
