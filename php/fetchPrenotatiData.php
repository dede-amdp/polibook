...
/* prende le tuple da 'risultato' che contengono la stessa matricola che è stata indicata, ovvero mostra all'utente gli esami ai quali è iscritto.*/
    ...
    $query = 'SELECT r.id_corso_esame_appello as corso, r.id_docente_esame_appello as id_docente, d.nome as docente, d.cognome as cognome_docente, a.id, a.nome, a.ordinamento, r.data_svolgimento_appello as \'data svolgimento\', ap.data_fine as \'disponibile fino al\', messaggio FROM risultato r JOIN appello ap JOIN attivita_didattica a JOIN docente d ON r.id_attdid_esame_appello= a.id AND r.ord_attdid_esame_appello=a.ordinamento AND d.id = r.id_docente_esame_appello AND
