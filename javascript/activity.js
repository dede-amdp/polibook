...
request('../php/fetchActivity.php', param).then(result => {
    if (result != undefined && result != null && result.length > 0) {
        const data = result[0];
        const titolo = data.id_attdid + ' - ' + data.nome_attdid;
        const cdl = data.id_cdl + ' - ' + data.nome_cdl;
        // costruisce info-generali
        const annocorso = 'Anno di corso:\t' + data.anno_corso + '</br>';
        ...
        const caratt = 'Caratterizzante per:\t' + (data.caratterizzante ? 'Sì' : 'No') + '</br>';
        ...
        document.getElementById('programma').innerHTML = `<a href='../php/download.php?id=${data.id_attdid}&ord=${data.ord_attdid}'>Programma ${data.nome_attdid}</a>`;
    } else {
